...
You're picking up work on **scrolls (aiskills)**, the agentic-memory ("scrolls") skill family shipped in the `sugatoray/aiskills` repo. Before making changes, read the following files **in this exact order**:
...
This directory (`src/scrolls/`) is **not** where the `scrolls-*` skill artifacts live — those are at `skills/scrolls/` (five skills: `scrolls-setup`, `scrolls-update`, `scrolls-hide`, `scrolls-unhide`, `scrolls-help`, each with its own `SKILL.md`, `meta/MAINTAINERS.md`, and `CHANGELOG.md`; `scrolls-help` additionally has `meta/SECURITY.md`). `src/scrolls/CLAUDE.md` is a hand-written pointer there, plus tested `npx skills add sugatoray/aiskills` install instructions. This `docs/.scrolls/` folder is this session's own dogfood application of the family's convention: it records the memory of the multi-session effort that hardened, restructured, and versioned the `scrolls-*` family itself. See `skills/scrolls/CHANGELOG.md` (symlinked at `src/scrolls/CHANGELOG.md`) for the authoritative, version-by-version record of what shipped in the skills themselves — this scroll set is the narrative/process memory around that work, not a duplicate of it.
