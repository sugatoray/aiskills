...
| `bibtex`: `` I was expecting a `{' or a `(' `` pointing at a `%`-prefixed line in `.bib` | A literal `@` character appears anywhere in `references.bib` before the entries start — including inside what looks like a `%` comment. BibTeX has no comment syntax of its own; it scans for the next `@` to start an entry regardless of a leading `%`, so a pandoc-style `[@key]` example written into a comment (exactly what happened in this template once) reads as a malformed entry | Remove or reword any literal `@` in `references.bib` outside of real `@type{...}` entries — spell it out ("an at-key citation") instead of writing the bracket syntax literally |
