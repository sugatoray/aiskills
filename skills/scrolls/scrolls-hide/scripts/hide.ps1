#!/usr/bin/env pwsh
# Renames scrolls folder(s) to .scrolls (dotfile-hidden), rewriting the
# path references inside the moved folder and in any SCROLLS.md that
# mentions it. Reports (never auto-edits) any other stray references it
# finds nearby.
#
# PowerShell port of hide.sh — the exact mirror of unhide.ps1 in the
# opposite direction; see that script's header for the full rationale.
...
#
# Usage:
#   hide.ps1 [-p ROOT | --path=ROOT | --path ROOT] ... [-r|--recurse]
#   hide.ps1 -t | --reporoot [-r|--recurse]
#   hide.ps1 -l | --local    [-r|--recurse]
